...
export const PostView = (props: PostWithUser) => {
  ...
  const jam = postCreatedAt.getHours();
  const menit = postCreatedAt.getMinutes();
  const tanggal = postCreatedAt.getDate();
  const bulan = postCreatedAt.toLocaleString("default", { month: "short" });
  const tahun = postCreatedAt.getFullYear();
  const amPm = jam >= 12 ? "PM" : "AM";
  const jam12 = (jam % 12 || 12).toString().padStart(2, "0"); // Memastikan 2 digit
  const menitFormatted = menit.toString().padStart(2, "0"); // Memastikan 2 digit
  ...
  return (
    <div
      ...
      }  w-full gap-3 border-b border-slate-700 px-5 py-3`}
    >
      ...
      <div className="flex flex-col gap-y-3 pt-4">
        {editPost || toggle ? (
          ...
          <span className="whitespace-pre-wrap text-[16px]">
            {post.content}
          </span>
        )}
        <Link href={`/post/${post.id}`} className="text-[#71767b]">
          {`${formattedDate}`}
        </Link>
        {/* Button Like */}
          ...
          <button
            ...
            className={`${
              post.likedByMe
                ...
                : "text-gray-500 hover:text-[#f91880]"
